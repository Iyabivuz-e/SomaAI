...
class ChatRequest(BaseModel):
    ...
    enable_analogy: Optional[bool] = Field(None, description="Include analogy in response")
    enable_realworld: Optional[bool] = Field(None, description="Include real-world context")
...
class CitationResponse(BaseModel):
    ...
    chunk_preview: str = Field(..., max_length=200, description="Preview of cited content")
...
class ChatResponse(BaseModel):
    ...
    sufficiency: bool = Field(..., description="Whether context was sufficient for answer")
    citations: List[CitationResponse] = Field(default_factory=list, description="Source citations")
    analogy: Optional[str] = Field(None, description="Analogy explanation if enabled")
    realworld_context: Optional[str] = Field(None, description="Real-world application if enabled")
...
class MessageResponse(BaseModel):
    ...
    session_id: Optional[str] = Field(None, description="Session ID if part of conversation")
